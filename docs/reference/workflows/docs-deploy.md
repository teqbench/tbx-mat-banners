...
Inherited from the calling repo via `secrets: inherit`. The reusable workflow defines the exact secrets it consumes (typically a GitHub App token for cross-repo writes to the docs target). Refer to the org-wide workflow for the authoritative list.
